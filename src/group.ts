  ...
  constructor(public readonly maxParallel: number) {
    ...
    this.parallelPromiseHandler = new ParallelPromiseHandler<Response>(
      ...
      (event, id, data, message) => {
        switch (event) {
          case 'enqueue':
            FetchLogger.logAdditionalAction(`Enqueued request ${id}`, data as RequestInitToolkit);
            break;
          case 'error':
            FetchLogger.logAdditionalAction(`Error on request ${id}: ${message}`, data as RequestInitToolkit);
            break;
        }
      },
    );
