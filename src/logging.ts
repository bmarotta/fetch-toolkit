...
export abstract class FetchLogger implements FetchDecorator {
  ...
  public static logAdditionalAction(message: string, request?: RequestInitToolkit) {
    if (request?.decorators) {
        ...
        const loggers = request.decorators.filter(decorator => decorator instanceof FetchLogger) as FetchLogger[];
  ...
  protected abstract logAction(logStr: string, action: 'Request' | 'Response' | 'Additional'): void;
}
